...
const knex = require("knex")({
  ...
  connection: {
    ...
    user: "joey_gorski", // Your user name
    password: "nobemaqo", // Your password
    database: "joey_gorski", // Your database name
  },
});
...

    {
      ...
      handler: async (request, h) => {
        const account = await Account.query()
          ...
          if(account && (await account.verifyPassword(request.payload.oldPassword))){
            const patched = await Account.query()
                                        .where("email", request.payload.email)
                                        .patch({password: request.payload.newPassword});
            if (patched) {
                return {
                    ok: true,
                    msge: `Successfully reset ${request.payload.email}'s password`,
                }
            }
            //else
            return {
              ok: false,
              msge: "Patch request for password failed",
            }
          }
        //else
        return {
          ok: false,
          msge: "Old password is incorrect or user does not exist",
        }
      },
    },
  ]);
